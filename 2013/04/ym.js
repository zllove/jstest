...
(function ($) {
    ...
    $.extend(String.prototype, {
        ...
        'toRealValue': function () {
            return(this == 'true' || this == 'false') ? this.toBoolean() : this.toNumber();
        },
        'trim': function () {
        ...
        'sliceAfter': function (str) {
            return(this.indexOf(str) >= 0) ? this.substring(this.indexOf(str) + str.length, this.length) : this.toString();
        },
        ...
        'parseAttrJSON': function () {
            ...
            for (var i = 0; i < a.length; i++) {
                ...
                var item = a[i].sliceBefore(':').trim(), val = a[i].sliceAfter(':').trim();
                if (item != '' && val != '')d[item.toCamelCase()] = val.toRealValue();
            }
            return d;
        }, 
...
(function ($) {
    ...
    $.extend(Date.prototype, {
        ...
        'format': function (format) {
            ...
            var o = {'m+': this.getMonth() + 1, 'd+': this.getDate(), 'h+': this.getHours(), 'n+': this.getMinutes(), 's+': this.getSeconds(), 'S': this.getMilliseconds(), 'W': ["日", "一", "二", "三", "四", "五", "六"][this.getDay()], 'q+': Math.floor((this.getMonth() + 3) / 3)};
...
(function ($) {
    ...
    $.ajaxq = function (queue, options) {
        ...
        if (typeof options != "undefined") {
            ...
            if (document.ajaxq.q[queue].length == 1)document.ajaxq.r = YM.dataproxy.ajax(options)
        } else {
            if (document.ajaxq.r) {
...
(function ($) {
    ...
    $.extend($.browser, {
        ...
        'isIPad': (/iPad/i).test(navigator.userAgent), 'language': (navigator.language || navigator.userLanguage || '').toLowerCase()
        });
    $.fn.tagName = function () {
    ...
    $.fn.bindJqueryUI = function (action, params) {
        ...
        var elm = this;
        YM.load('jqueryui', function () {
            elm[action](params);
        });
        return this;
    };
    $.fn.bindYMUI = function (type, params, file) {
        if (this.size() == 0 || !YM)return this;
        if (YM.ui && YM.ui[type]) {
            YM.ui[type](this, params);
            this.data(type + '-binded', true);
        } else {
            this.bindYMUIExtend(file || 'ui', type, params);
        }
        return this;
    };
    $.fn.bindYMUIExtend = function (file, type, params) {
        if (this.size() == 0 || !YM)return this;
        var elm = this;
        YM.load(file, function () {
            setTimeout(function () {
                if (!YM.ui[type])return;
                YM.ui[type](elm, params);
...
(function ($) {
    ...
    YM.namespace = function (name, sep) {
    ...
    YM.debugMode = false;
    YM.debugIndex = 0;
    YM.debug = function (a, b) {
        if (!this.debugMode)return;
        if (typeof(console) == 'undefined') {
            YM.debug.log(((Date.prototype.format) ? (new Date()).format('hh:nn:ss.S') : (++YM.debugIndex)) + ', ' + a + ' = ' + b);
        } else {
            if (console && console.log)console.log(((Date.prototype.format) ? (new Date()).format('hh:nn:ss.S') : (++YM.debugIndex)) + ', ' + a, '=', b);
        }
    };
    YM.debug.log = function () {
        this.createDOM();
        var p = [], v = $('#_ym_debuglog textarea').val();
    ...
    YM.debug.clear = function () {
        $('#_ym_debuglog textarea').val('');
    };
    YM.debug.createDOM = function () {
    ...
    YM.load = function (service, action, params) {
        if ($.isArray(service)) {
            var url = service.join(',');
            var urlsize = service.length;
            var status = YM.loader.checkFileLoader(url);
            if (status == urlsize + 1) {
                if (typeof(action) == 'function')action();
            } else if (status > 0) {
                YM.loader.addExecute(url, action);
            } else if (status == 0) {
                YM.loader.addExecute(url, action);
                YM.loader.fileLoader[url] = 1;
                YM.debug('开始加载JS', url);
                for (var i = 0; i < urlsize; i++) {
                    YM.load(service[i], function () {
                        YM.loader.fileLoader[url]++;
                        if (YM.loader.fileLoader[url] == urlsize + 1) {
                            YM.debug('完成加载JS', url);
                            YM.loader.execute(url);
                        }
                    });
                }
            }
        } else if (YM.loader.serviceLibs[service] && YM.loader.serviceLibs[service].requires) {
            YM.load(YM.loader.serviceLibs[service].requires, function () {
                YM.load.run(service, action, params);
            });
        } else {
            YM.load.run(service, action, params);
        }
    ...
    YM.load.version = '';
    YM.load.add = function (key, data) {
        if (YM.loader.serviceLibs[key])return;
        ...
        YM.loader.serviceLibs[key] = data;
    };
    YM.load.setPath = function (path) {
        YM.loader.serviceBase = path;
    };
    YM.load.run = function (service, act, params) {
        var action = (typeof(act) == 'string') ? (function () {
            try {
                var o = eval('YM.' + service);
                if (o && o[act])o[act](params);
            } catch (e) {
            }
        }) : (act || function () {
        });
        if (YM.loader.checkService(service)) {
            action();
            return;
        }
        var url = YM.loader.getServiceUrl(service);
        var status = YM.loader.checkFileLoader(url);
        if (status == 2) {
            action();
        } else if (status == 1) {
            ...
            if ($('script[src="' + url + '"]').size() > 0) {
                YM.loader.fileLoader[url] = 2;
                action();
            } else {
                YM.loader.addExecute(url, action);
                YM.loader.addScript(service);
            }
        } else {
            YM.debug('加载异常', service);
        }
    };
    YM.loader = {};
    YM.loader.fileLoader = {};
    YM.loader.executeLoader = {};
    YM.loader.serviceBase = (function () {
        return $('script:last').attr('src').sliceBefore('/js/') + '/';
    })();
    YM.loader.serviceLibs = {};
    YM.loader.checkFullUrl = function (url) {
        return(url.indexOf('/') == 0 || url.indexOf('http://') == 0);
    };
    YM.loader.checkService = function (service) {
    ...
    YM.loader.checkFileLoader = function (url) {
        return(url != '') ? (this.fileLoader[url] || 0) : -1;
    };
    YM.loader.getServiceUrl = function (service) {
        var url = '';
    ...
    YM.loader.execute = function (url) {
    ...
    YM.loader.getScript = function (url, onSuccess, onError) {
        this.getRemoteScript(url, onSuccess, onError);
        this.fileLoader[url] = 1;
        YM.debug('开始加载JS', url);
    };
    YM.loader.getRemoteScript = function (url, param, onSuccess, onError) {
        if ($.isFunction(param)) {
            onError = onSuccess;
            onSuccess = param;
            param = {};
    ...
    YM.timestat = {};
    YM.timestat.libs = {};
    YM.timestat.loadTime = (typeof(_ym_page_loadtime) == 'number') ? _ym_page_loadtime : new Date().getTime();
    YM.timestat.add = function (name) {
        this.libs[name] = new Date().getTime() - this.loadTime;
    };
    YM.timestat.get = function (name) {
        return this.libs[name] || 0;
    };
    YM.lang = {};
    YM.lang.language = 'zh-cn';
    YM.lang.text = {};
    YM.lang.get = function (dataset, name) {
        if (name) {
    ...
    YM.lang.set = function (dataset, name, value) {
        if (!this.text[dataset]) {
            this.text[dataset] = {};
        }
        if (value) {
    ...
    YM.lang.extend = function (dataset, data) {
        if (!this.text[dataset]) {
            this.text[dataset] = {};
        }
...
(function ($) {
    if (!$ || !window.YM)return;
    YM.namespace('YM.json');
    YM.json.parse = function (data) {
        return(new Function("return " + data))();
    };
    YM.json.stringify = function (obj) {
...
(function ($) {
    if (!$ || !window.YM)return;
    YM.namespace('YM.cookie');
    YM.cookie.getRootDomain = function () {
        var domain = document.domain;
        if (domain.indexOf('.') > 0 && !YM.valid.isIP(domain)) {
            var k = domain.split('.'), d1 = k[k.length - 1], d2 = k[k.length - 2], d3 = k[k.length - 3];
            domain = (d2 == 'com' || d2 == 'net') ? (d3 + '.' + d2 + '.' + d1) : (d2 + '.' + d1);
        }
        return domain;
    };
    YM.cookie.get = function (name) {
        var cname = name + '=';
    ...
    YM.cookie.del = function (name, root) {
        var domain = (root) ? ('; domain=' + this.getRootDomain()) : '';
        document.cookie = name + '=; path=/' + domain + '; expires=Thu,01-Jan-70 00:00:01 GMT';
    };
    YM.cookie.set = function (name, value, expdate, root) {
        var exp = new Date(), domain = (root) ? ('; domain=' + this.getRootDomain()) : '';
        if (typeof(expdate) == 'null' || typeof(expdate) == 'undefined') {
            expdate = 365;
        }
        exp.setTime(exp.getTime() + (1000 * 60 * 60 * 24 * expdate));
...
(function ($) {
    ...
    YM.valid.isCardNo = function (str, cardType) {
        var cards = {"Visa": {lengths: "13,16", prefixes: "4", checkdigit: true}, "MasterCard": {lengths: "16", prefixes: "51,52,53,54,55", checkdigit: true}, "BankCard": {lengths: "16,17,19", prefixes: "3,4,5,6,9", checkdigit: false}};
        if (!cards[cardType])return false;
        var cardNo = str.replace(/[\s-]/g, "");
...
(function ($) {
    if (!$ || !window.YM)return;
    YM.namespace('YM.page');
    YM.page.setHomepage = function () {
    ...
    YM.page.addFavor = function (u, t) {
        var url = u || document.location.href, title = t || document.title;
        if (url.match(/_fromid=[\w]*/)) {
            url = url.replace(/_fromid=[\w]*/, '_fromid=userfavorite');
        } else {
            url += ((url.indexOf('?') > 0) ? '&' : '?') + '_fromid=userfavorite';
    ...
    YM.page.refresh = function (url) {
        var url = url || document.location.href.sliceBefore('#');
        document.location.href = url;
    };
    YM.page.setDomain = function () {
        var d = document.domain;
        if (d.indexOf('.') < 0 || YM.valid.isIP(d))return;
        var k = d.split('.'), d1 = k[k.length - 1], d2 = k[k.length - 2], d3 = k[k.length - 3];
        document.domain = (d2 == 'com' || d2 == 'net') ? (d3 + '.' + d2 + '.' + d1) : (d2 + '.' + d1);
    };
    YM.page.keyHandler = {};
    YM.page.keyHandler.events = {};
    YM.page.keyHandler.keys = {'ESC': 27, 'PAGEUP': 33, 'PAGEDOWN': 34, 'END': 35, 'HOME': 36, 'LEFT': 37, 'TOP': 38, 'RIGHT': 39, 'DOWN': 40, 'INSERT': 45, 'DELETE': 46, 'F1': 112, 'F2': 113, 'F3': 114, 'F4': 115, 'F5': 116, 'F6': 117, 'F7': 118, 'F8': 119, 'F9': 120, 'F10': 121, 'F11': 122, 'F12': 123};
    YM.page.keyHandler.add = function (doc, key, eventItem, eventCallback) {
        this.events[eventItem] = function (e) {
            try {
                var code = e.which || e.keyCode || 0;
                if (code == YM.page.keyHandler.keys[key]) {
                    eventCallback();
                }
            } catch (err) {
            }
    ...
    YM.page.keyHandler.remove = function (doc, eventItem) {
        $(doc).unbind('keydown', this.events[eventItem]);
        this.events[eventItem] = null;
    };
    YM.page.flash = function (pSwf, pBoxID, pW, pH, pVer, pSetup, pFlashvars, pParams, pAttr) {
        YM.load('swfobject', function () {
            swfobject.embedSWF(pSwf, pBoxID, pW, pH, pVer, pSetup || false, pFlashvars || false, pParams || false, pAttr || false);
        });
    };
    YM.page.msgtip = function (source, content, param) {
        var param = param || {};
        param.content = content;
        YM.load('util-msgtip', function () {
            new YM.util.msgtip($(source), param);
        });
    };
    YM.page.popbox = function (param) {
        YM.load('util.popbox', 'show', param);
    };
    YM.page.popbox.close = function () {
        YM.load('util.popbox', 'close', null);
    };
    YM.page.alert = function (message, callback) {
        if (YM.util && YM.util.dialog && YM.util.dialog.status) {
            alert(message);
            if (callback)callback();
            return false;
        } else {
            var params = ($.isPlainObject(message)) ? message : {message: message, callback: callback};
            YM.load('util.dialog', 'alert', params);
        }
    };
    YM.page.showSuccess = function (message, callback) {
        var params = ($.isPlainObject(message)) ? message : {message: message, callback: callback};
        params.type = 'success';
        YM.load('util.dialog', 'alert', params);
    };
    YM.page.showError = function (message, callback) {
        var params = ($.isPlainObject(message)) ? message : {message: message, callback: callback};
        params.type = 'error';
        YM.load('util.dialog', 'alert', params);
    };
    YM.page.confirm = function (message, callback, cancel) {
        var params = ($.isPlainObject(message)) ? message : {message: message, callback: callback, cancel: cancel};
        YM.load('util.dialog', 'confirm', params);
    };
    YM.page.prompt = function (message, callback) {
        var params = ($.isPlainObject(message)) ? message : {message: message, callback: callback};
        YM.load('util.dialog', 'prompt', params);
    };
    YM.page.showLoading = function (message) {
        var params = ($.isPlainObject(message)) ? message : {loadingText: message};
        YM.load('util.dialog', 'showLoading', params);
    };
    YM.page.hideLoading = function () {
        YM.load('util.dialog', 'close', '');
    };
    YM.page.dialog = function (params) {
        YM.load('util.dialog', 'open', params);
    };
    YM.page.dialog.pop = function (params) {
        YM.load('util.dialog', 'pop', params);
    };
    YM.page.dialog.poptab = function (tabs, width, height) {
        var params = tabs[0];
        $.extend(params, {tabs: tabs, width: width, height: height});
        YM.load('util.dialog', 'pop', params);
    };
    YM.page.dialog.show = function (params) {
        YM.load('util.dialog', 'show', params);
    };
    YM.page.dialog.ajax = function (params) {
        YM.load('util.dialog', 'ajax', params);
    };
    YM.page.dialog.form = function (frm, title, width, height) {
        ...
        params.content = tmpl.substitute({frameId: frameId});
        params.callback = function () {
            frm.target = frameId;
            frm.submit();
        };
        YM.page.dialog(params);
        return false;
    };
    YM.page.closeDialog = function () {
        YM.load('util.dialog', 'close', '');
    };
    ...
    YM.page.dialog.setConfig = function (attr, value) {
        YM.load('util.dialog', function () {
            YM.util.dialog[attr] = value;
        });
    };
})(jQuery);
// --------------------------------
...
(function ($) {
    if (!$ || !window.YM)return;
    YM.namespace('YM.login');
    YM.login.servicePath = '/';
    YM.login.serviceAPI = {'login': 'portal/toLogin.jspa', 'register': 'inc/register.jsp', 'headinfo': 'top/jsonHeaderInfoWithCart.jspa?callback=?'};
    YM.login.setServicePath = function (path) {
        this.servicePath = path;
    };
    YM.login.loadTime = new Date().getTime();
    YM.login.headData = {};
    YM.login.userData = {};
    YM.login.getUserInfo = function (callback) {
        this.addEvent(callback);
        this.loadUserInfo();
    };
    YM.login.loadUserInfo = function (callback) {
        var callback = callback || function () {
            YM.login.exeEvents();
        };
        this.loadTime = new Date().getTime();
        $.getJSON(this.servicePath + this.serviceAPI.headinfo, function (data) {
            YM.login.headData = data;
            YM.login.userData = {};
            if (data.memberInfo) {
                YM.login.userData = data.memberInfo;
                YM.login.userData.nickName = YM.login.userData.nickName || YM.login.userData.logName;
                YM.login.userData.isLogin = true;
            }
            callback(YM.login.userData, YM.login.headData);
            if (YM.login.userData.isLogin) {
                YM.login.clearEvents();
            }
        });
    };
    YM.login.checkUserStatus = function () {
        var m = (new Date().getTime() - this.loadTime) / 60000;
        if (this.userData.isLogin && m < 5)return true;
        return false;
    };
    YM.login.events = [];
    YM.login.addEvent = function (callback) {
        if (callback)this.events.push(callback);
    };
    YM.login.exeEvents = function () {
        for (var i = 0; i < this.events.length; i++) {
            this.events[i](this.userData, this.headData);
        }
    };
    YM.login.clearEvents = function () {
        this.events = [];
    };
    YM.login.pop = function (param, callback) {
        if (!callback) {
            callback = param;
            param = {};
        }
        ...
        if (this.checkUserStatus()) {
            this.callback(this.userData, this.headData);
        } else {
            this.loadUserInfo(function (userData, headData) {
                if (userData.isLogin) {
                    YM.login.callback(userData, headData);
                } else {
                    YM.login.dialog(param);
                }
            });
        }
        return false;
    };
    YM.login.dialog = function (param) {
        if (this.servicePath != '/') {
            YM.page.setDomain();
            param.crossdomain = true;
        }
        YM.page.dialog.poptab([
            {title: '登&nbsp;&nbsp;录', url: this.servicePath + this.serviceAPI.login + '?' + $.param(param)},
            {title: '快速注册', url: this.servicePath + this.serviceAPI.register + '?' + $.param(param)}
        ], 400, 400);
        this.status = true;
        this.success = function () {
            YM.page.closeDialog();
            YM.login.addEvent(YM.login.callback);
            YM.login.status = false;
            YM.login.getUserInfo();
        };
    };
    YM.login.success = function (url) {
        document.location.href = url;
    };
    YM.login.status = false;
})(jQuery);
...
(function ($) {
    if (!$ || !window.YM)return;
    YM.namespace('YM.dataproxy');
    YM.dataproxy.loaders = [];
    YM.dataproxy.add = function (callback) {
        if (this.j$) {
            callback(this.j$);
            return;
        }
        if (this.loaders.length == 0) {
            YM.page.setDomain();
            $('body').append('<iframe name="dataProxy" id="dataProxy" src="http://www.yesmywine.com/inc/domainProxy.jsp?crossdomain=true" width="0" height="0" style="display:none;"></iframe>');
        }
        this.loaders.push(callback);
    };
    YM.dataproxy.setJquery = function (j$) {
        this.j$ = j$;
        for (var i = 0; i < this.loaders.length; i++) {
            this.loaders[i](this.j$);
        }
        this.loaders = [];
    };
    YM.dataproxy.ajax = function (params) {
        if (YM.login.servicePath == '/') {
            $.ajax(params);
        } else {
            this.add(function (j$) {
                j$.ajax(params);
    ...
    YM.dataproxy.get = function (url, params, success, type) {
        if (YM.login.servicePath == '/') {
            $.get(url, params, success, type);
        } else {
            this.add(function (j$) {
                j$.get(url, params, success, type);
            });
        }
    };
    YM.dataproxy.getJSON = function (url, params, success) {
        if (YM.login.servicePath == '/') {
            $.getJSON(url, params, success);
        } else {
            this.add(function (j$) {
                j$.getJSON(url, params, success);
            });
        }
    };
    YM.dataproxy.post = function (url, params, success, type) {
        if (YM.login.servicePath == '/') {
            $.post(url, params, success, type);
        } else {
            this.add(function (j$) {
                j$.post(url, params, success, type);
            });
        }
    };
})(jQuery);
...
(function ($) {
    ...
    YM.load.add('lhgcalendar', { js: 'js/lib/lhgcalendar/lhgcalendar.min.js' });
