...
jQuery.Callbacks = function( options ) {
	...
	var // Flag to know if list is currently firing
		...
		self = {
			// Add a callback or a collection of callbacks to the list // 回调列表中添加一个回调或回调的集合。
			add: function() {
				if ( list ) {
					// First, we save the current length
					var start = list.length; //首先我们存储当前列表长度
					(function add( args ) { //jQuery.each，对args传进来的列表的每一个对象执行操作
						jQuery.each( args, function( _, arg ) {
							...
							} else if ( arg && arg.length && type !== "string" ) { //如果是类数组或对象,递归
								// Inspect recursively
								add( arg );
							}
						});
					...
					} else if ( memory ) { //如果options.memory为true，则将memory做为参数，应用最近增加的回调函数
						firingStart = start;
						fire( memory );
					}
				}
...
jQuery.event = {
	...
	// 目的就是为填充events，eventHandle
	// 并给选中元素注册事件处理程序
	add: function( elem, types, handler, data, selector ) {
		var tmp, events, t, handleObjIn,
			special, eventHandle, // 不仅仅只是只是充当一个回调函数的角色，而是一个实现了EventListener接口的对象
			handleObj,
			handlers, type, namespaces, origType,
			elemData = jQuery._data( elem ); // 1. 在$.cahce缓存中获取存储的事件句柄对象，如果没就新建elemData
		...
		while ( t-- ) {
			...
			// 这里把handleObj叫做事件处理对象,扩展一些来自handleObjIn的属性
    ...
    */
	trigger: function( event, data, elem, onlyHandlers ) {
		var handle, ontype, cur,
			...
			type = hasOwn.call( event, "type" ) ? event.type : event, // 指定事件类型
		...
		event.isTrigger = onlyHandlers ? 2 : 3; //开关，表示已经使用了trigger (触发器)
	...
	fixHooks: {}, // 缓存不同事件所属的事件类别

	keyHooks: {
		props: "char charCode key keyCode".split(" "), // 存储键盘事件的特有属性
...
jQuery.Event = function( src, props ) { // 增加自己的处理机制呗，这样更灵活，而且还可以传递data数据，也就是用户自定义的数据
...
jQuery.Event.prototype = {
	...
	preventDefault: function() {
		...
		this.isDefaultPrevented = returnTrue; // 加了一个状态机用来记录，当前是否调用过这个方法
		if ( !e ) {
			return;
		}
