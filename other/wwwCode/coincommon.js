...
function showlogin(id){
	...
	jQuery("#okcoinPop").load("/login.do?type="+id,function (data){
		dialogBoxShadow();
		showDialog(id);
	});
}
...
function okcoinAlert(str,pro,callback,btnTitle) {
	...
	*/
		...
		tempStr.push('<div class="orderFloorTitle"><span style="float:left;">&nbsp;&nbsp;Okcoin提示</span><span style="float:right;"><a id="alertClose" href="javascript:void(0);" class="dialog_closed" title="关闭"></a></span></div>');
...
var availableTagsDef = ["@qq.com","@163.com","@126.com","@sina.com","@gmail.com","@sohu.com","@vip.qq.com","@hotmail.com","@163.net","@sina.com.cn","@139.com","@189.cn"];
var availableTags = ["@qq.com","@163.com","@126.com","@sina.com","@gmail.com","@sohu.com","@vip.qq.com","@hotmail.com","@163.net","@sina.com.cn","@139.com","@189.cn"];
...
function loginSubmit(){
	if(checkLoginUserName() && checkLoginPassword()){
		var url = "/login/index.do?random="+Math.round(Math.random()*100);
				...
				if(result!=null){
					if(result.resultCode == 0){
						if(document.getElementById("forwardUrl")!=null && document.getElementById("forwardUrl").value != ""){
							...
							if(whref.length < 30){
								whref = document.getElementById("coinMainUrl").value+"/btc.do";
							}
...
function checkRegUserName(){
	...
	var url = "/reg/chcekregname.do?name=" + encodeURI(regUserName) +"&type="+regType+"&random="+Math.round(Math.random()*100);
...
function checkPassword(){
	...
	function regSubmit(){
		...
		if(checkRegUserNameNoJquery() && checkPassword() && checkRePassword() ){
			...
			var regType = document.getElementById("regType").value;
			var pwd = trim(document.getElementById("regPassword").value);
			var urlName = "/reg/chcekregname.do?name=" + encodeURI(regUserName) +"&type="+regType+"&random="+Math.round(Math.random()*100);
			jQuery.get(urlName,null,function(data){
				if(data == 0){
					...
					var url = "/reg/index.do?random="+Math.round(Math.random()*100);
					var param={regName:regUserName,password:pwd,regType:regType};
					jQuery.post(url,param,function(data){
						if(data < 0){
...
/*********微博登录**********************/
	function openss(url){
	    ...
	    window.open('/weibo/call.do?url='+url,'new','height='+450+',,innerHeight='+450+',width='+550+',innerWidth='+550+',top='+200+',left='+200+',toolbar=no,menubar=no,scrollbars=auto,resizeable=no,location=no,status=no');
	}
/*********QQ登录*******************************/	
	function openqq(url){
		if(url==null||url==""){
		...
		window.open('/qq/call.do?url='+url,'new','height='+550+',,innerHeight='+550+',width='+600+',innerWidth='+600+',top='+200+',left='+200+',toolbar=no,menubar=no,scrollbars=auto,resizeable=no,location=no,status=no');
	}
	...
	function handleTicker(){
		var url = "/ticker.do?random="+Math.round(Math.random()*100);
...
function bindAuth(){
	var callback={okBack:function(){window.location.href= document.getElementById("coinMainUrl").value+"/security.do";},noBack:function(){return false;}};
